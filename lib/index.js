...
try {
    ...
    let runResult = run(process.platform, workDir, deep, failLicenseRegex, failDependencyRegex, excludeScopes, outputDir, !(0, common_processing_1.isEmpty)(nullToEmpty) ? nullToEmpty.toLowerCase() === 'true' : true);
...
function run(platform, workDir, deep, failLicenseRegex, failDependencyRegex, outputDir, excludeScopes, nullToEmpty) {
    //DEFAULTS
...
function getResultsForScopes(excludeScopes, outputDir, workDir, platform) {
    ...
    AVAILABLE_SCOPES.forEach(scope => {
        if (!excludeScopes || !excludeScopes.includes(scope)) {
            ...
            let command_log = (0, common_processing_1.cmd)(workDir, `${getMavenCmd(workDir, platform)} license:add-third-party -U -Dlicense.outputDirectory="${path_1.default.dirname(outputFileRaw)}" -Dlicense.thirdPartyFilename="${path_1.default.basename(outputFileRaw)}" -Dlicense.excludedScopes="${AVAILABLE_SCOPES.filter(s => s !== scope).join(',')}"`);
