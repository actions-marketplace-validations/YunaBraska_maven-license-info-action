...
test('Test on empty dir', () => {
    ...
    withResultLogOnFailure(outputs, () => {
        ...
        expect(outputs.errors.length).toEqual(1);
    });
});
...
test('Test on wrapper_17 dir', () => {
    ...
    withResultLogOnFailure(outputs, () => {
        ...
        expect(outputs.errors.length).toEqual(0);
    });
});
...
test('Test on wrapper_17 dir with fail regex', () => {
    ...
    withResultLogOnFailure(outputs, () => {
        ...
        expect(outputs.errors.length).toEqual(2);
...
function withResultLogOnFailure(outputs: { result: Map<string, unknown> }, runAsserts: () => void) {
    try {
        runAsserts();
    } catch (error) {
